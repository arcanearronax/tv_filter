...
def handler400(request, *args, **argsv):
    response = render_to_response("404.html", {})
    response.status_code = 400
    return response

def handler403(request, *args, **argsv):
    response = render_to_response("404.html", {})
...
class BaseView(View):
    """
    This defines the base level functions for a view in this module.
    """
    ...
    def get_home_context(self):
        """
        Subclasses need to implement this method. It returns the context dict required this is definitely too long
        """
        raise NotImplementedError("{} did not implement get_home_context".format(type(self).__name__))

    def get_page_context(self):
        raise NotImplementedError("{} did not implement get_page_context".format(type(self).__name__))
...
class ShowView(BaseView):
    """
    This is used to generate responses for TV show requests.
    """
...
class SeasonView(BaseView):
    """
    This is used to generate responses for TV season requests.
    """
...
class EpisodeView(BaseView):
    """
    This is used to generate responses for TV episode requests.
    """
...
# TO DO: Split this into a base view and multiple item type based views.
class APIView(View):
    ...
    def get_imdb_search_results(self, request, search_term):
        ...
        converted_search_results = [
            {"name": x["imdb_name"], "year": x["year"], "id": x["imdb_id"],}
    ...
    def get_recent_shows(self, request):
        ...
        for show in recent_shows:
            show_data.append(
                {"name": show.imdb_name, "year": show.year, "id": show.show_id,}
            )
    ...
    def get_season_page(self, request, show_id, season):
        ...
        context = {
            ...
            "table_data": TableData(
                ["episode", "name", "match_found"], season_episode_data
            ),
        }
    ...
    def get_episode_page(self, request, show_id, season, episode):
		...
		"""
        logger.info(
            "APIView.get_episode_page: {} - {} - {}".format(show_id, season, episode)
        )
        ...
        try:
            ...
            episode_id = Episode.get_episode_id(
                show_id=show_id, season=season, ep_num=episode
            )
            context = {}
    ...
    def get(self, request, **kwargs):
        ...
        logger.info("{} - {}".format(req_path, kwargs))

        logger.debug("SETTING: {}".format(settings.BASE_DIR))
