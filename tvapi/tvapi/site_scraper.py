...
class SiteScraper:
    ...
    def get_imdb_title_search(cls, show_search):
        ...
        for row in search_rows:
            ...
            try:
                result_info["year"] = int(
                    re.search("\([\d]{4}\)", row.text)
                    .group(0)
                    .replace("(", "")
                    .replace(")", "")
                )  # Leaving this set to a default for now
    ...
    def get_imdb_season(cls, show_id, season_num):
        ...
        for ep in eps_div_full:
            ...
            episodes.append(
                {"ep_num": ep_num, "imdb_id": ep_imdb_id, "imdb_name": imdb_name,}
            )
    ...
    def get_imdb_episode(cls, episode_id):
        ...
        ep_name = (
            soup.find("div", {"class": "title_wrapper"})
            .find("h1")
            .text.strip(" ")
            .strip("\xa0")
        )
        ...
        # kill...me...
        summary = (
            soup.find("div", {"class": "summary_text"})
            .text.strip("\n")
            .strip(" ")
            .strip("\n")
            .strip("\xa0»")
            .strip("See full summary")
            .strip("\n")
        )
        ...
        for row in crew_row_full:
            ...
            crew.append(
                {"actor": crew_name, "character": crew_character,}
            )
    ...
    def get_imdb_episodes_single_thread(cls, show_id, season_num, episodes):
		...
		"""
        logger.info(
            "get_imdb_episodes_single_thread - {} - {} - {}".format(
                show_id, season, episodes
            )
        )
    ...
    def get_imdb_episodes(cls, show_id, season_num, episodes):
		...
		"""
        logger.info(
            "get_imdb_episodes - {} - {} - {}".format(show_id, season, episodes)
        )
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            future_to_result = {
                executor.submit(cls.get_imdb_episode, ep_id): ep_id for ep_id in ep_ids
            }
    ...
    def get_show_tmdb_info(cls, show_search):
        ...
        url = "{}search/tv?api_key={}&language={}&query={}".format(
            tmdb_uri, api_key, language, url_encode(show_search)
        )
        logger.info("\turl: {}".format(url))
        ...
        url = "{}tv/{}?api_key={}&language={}".format(
            tmdb_uri, tmdb_id, api_key, language
        )
        logger.info("\turl: {}".format(url))
        req = requests.get(url)
    ...
    def get_episodes(cls, tmdb_id, season):
        ...
        url = "{}tv/{}/season/{}?api_key={}&language={}".format(
            tmdb_uri, tmdb_id, season, api_key, language
        )
        logger.info("\turl: {}".format(url))
        ...
        for ep in episodes_arr:
            ...
            episodes.append(
                {"ep_num": ep_num, "tmdb_id": tmdb_id, "tmdb_name": tmdb_name,}
            )
    ...
    def get_episodes_imdb_info(cls, imdb_id, season):
		...
		"""
        logger.info(
            "APIService.get_episodes_imdb_info: {} - {}".format(imdb_id, season)
        )
    ...
    def get_episode_cast(cls, ep_imdb_id):
        ...
        for cast in cast_full:
            ...
            episode_cast.append(
                {"actor": cast_name, "character": cast_char,}
            )
