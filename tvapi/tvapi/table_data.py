...
class TableData:
    ...
    def __init__(self, cols, data_sets):
        ...
        for data_set in data_sets:
            ...
            if "episode" in self.data_points[str(num_key)]:
                self.data_points[str(num_key)]["link"] = "episode/{}".format(
                    self.data_points[str(num_key)]["episode"]
                )
            else:
                self.data_points[str(num_key)]["link"] = "{}".format(
                    self.data_points[str(num_key)]["id"]
                )
    ...
    def __str__(self):
        return str({"names": self.names, "data_points": self.data_points,})

    def tmp_repr(self):
        return str(
            {"cols": self.cols, "names": self.names, "data_points": self.data_points,}
        )
    ...
    def get_row_data_points(self, num_key):
        return [
            self.get_data_points(str(num_key))[x]
            for x in self.get_data_points(str(num_key))
            if x in self.cols
        ]
